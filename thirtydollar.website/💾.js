...
function quickSave() {
    ...
    let saveData = generateSequenceFile()
    if (!saveData.length) return
    if (ctrlHeld && altHeld) return openSequencePreview(new Blob([saveData], {
        type: 'text/txt;charset=UTF-8'
    }))
...
function loadSequence(data) {
    ...
    try {
        ...
        (data || "").replace(/\s/g, "").split("|").forEach(x => {
            ...
            if (element)
                for (let i = 0; i < (count || 1); i++) sequenceData += (element[0].outerHTML)
        })
...
$('#sequence').on('DOMSubtreeModified', function() {
