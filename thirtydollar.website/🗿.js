...
if (mobile) $('p[mobile]').each(function() {
    $(this).text($(this).attr('mobile'));
    $('.nomobile').hide()
})

const intros = [{
        name: $('#caption').text(),
        path: "assets/dont_you_lecture_me.wav"
    },
    {
        name: "how you gonna talk behind my back when you deadass built like a",
        path: "assets/you_deadass_built_like_a.wav"
    },
    {
        name: "white people be like",
        path: "assets/white_people_be_like.wav"
    }
]
...
const actions = [{
    ...
    },
    {
        ...
        amount: true,
        name: "volume",
        image: "assets/action_volume.png",
        default: 100,
        set: [0, 600, 1, "%"],
        add: [-600, 600, 1, "%"],
        multiply: [0.01, 1000, 0.1]
    ...
    {
        shortcut: "p",
        action: "Pause for duration",
        amount: true,
        name: "stop",
        image: "assets/action_stop.png",
        default: 4,
        set: [0, 1000]
    },
    {
        ...
        amount: true,
        name: "transpose",
        image: "assets/action_transpose.png",
        default: 1,
        set: [-60, 60],
        add: [-60, 60],
        multiply: [0.01, 100, 0.1]
    },

    {
        ...
        amount: true,
        name: "loopmany",
        image: "assets/action_loopmany.png",
        default: 4,
        set: [1, 1000]
    },
    {
        ...
        action: "Loop once",
        name: "loop",
        image: "assets/action_loop.png"
    },
    {
        shortcut: "s",
        action: "Set loop start point",
        name: "looptarget",
        image: "assets/action_looptarget.png"
    },
    {
        shortcut: "c",
        action: "Combine sounds",
        name: "combine",
        image: "assets/action_combine.png"
    },
    ...
    },
    {
        ...
        isTarget: true,
        name: "target",
        image: "assets/action_target.png",
        set: [1, 9999]
    },
    {
        shortcut: "x",
        ...
        name: "cut",
        stopSounds: true,
        image: "assets/action_cut.png"
    },
    {
        shortcut: "o",
        action: "Set start position",
        name: "startpos",
        image: "assets/action_startpos.png"
    },

    {
        ...
        action: "Add divider",
        name: "divider",
        image: "assets/action_divider.png"
    },
    {
        shortcut: "f",
        action: "Flash screen",
        name: "flash",
        image: "assets/action_flash.png"
    },
    {
        shortcut: "u",
        action: "Pulse screen",
    ...
    },
    {
        ...
        default: ["X", 1],
        name: "bg",
        image: "assets/action_bg.png"
    }
]
actions.forEach(x => {
    $('#actions').append(`<div class="action" action="${x.name}" info="${x.action}" key="${x.shortcut.toUpperCase()}"><img action="${x.name}" alt="${x.action}" src="${x.image}">${x.amount ? "<p>+</p>" : ""}</div>`)
})
...
$(document).on('click contextmenu', '.iconbox, #proHotbar', function() {
    return false
})
...
$(document).on('click contextmenu', '#icons div, #hotbarNotes div.sound', function(event) {
    ...
    if (!sound.startsWith("_")) {
        ...
        playSound(sound, {
            pitch: semitonesToPercent(pitch)
        })
    }
...
function displaySection(scroll) {
    ...
    if (selectedDivider >= 0) {
        ...
        if (foundDivider.hasClass("sectionHidden")) {
            $('#hideSection').hide();
            $('#showSection').show()
        } else {
            $('#hideSection').show();
        ...
        if (!active && scroll) $('#everything').stop().animate({
            scrollTop: foundDivider.prop("offsetTop") - 150
        }, 100)
    } else {
...
$(document).on('wheel touchmove', '#sequence div', function(event) {
    ...
    if (el.attr("sound") && !el.attr("str").startsWith("_")) {
        ...
        if (!isVolume) {
            ...
            el.attr("pitch", shift)
            playSound(el.attr("sound"), {
                pitch: semitonesToPercent(shift),
                volume: getVolume(el),
                stopPrevious: true
            })
        } else {
...
// prevent ctrl+zoom
$('.iconbox, #proHotbar').bind('mousewheel DOMMouseScroll', function(e) {
...
function playIntro() {
    stopIntro()
    let introSound = new AudioBufferSourceNode(soundcloud, {
        buffer: lecture
    })
...
document.addEventListener('scroll', function(event) {
    if ($(event.target).is("#sequence div")) $(event.target).scrollTop(200)
}, true);
$('#sequence').on('DOMSubtreeModified', function(event) {
    $('#sequence div').scrollTop(200)
});
...
function percentToSemitones(percent) {
    return Math.log(percent, 2) * 12
}
...
function getPitch(element) {
    return semitonesToPercent(element.find('p').text() || "0")
}

function getVolume(element) {
    return Number(element.find('vol').text().slice(0, -1) || 100) / 200
}
...
function cancel(cancelOptions = {}) {
    ...
    $('#sequence p[triggeredCountdown]').each(function() {
        resetAmount($(this))
    })
...
async function playSound(name, soundSettings = {}) {
    ...
    let snd = new AudioBufferSourceNode(soundcloud, {
        buffer: sounds[name],
        playbackRate: soundSettings.pitch || 1
    })
    let vol = new GainNode(soundcloud, {
        gain: !isNaN(soundSettings.volume) ? soundSettings.volume : 0.5
    })
    ...
    activeSounds.push({
        sound: snd,
        gainNode: vol,
        index: soundSettings.index || 0,
        name,
        pitch: soundSettings.pitch || 1,
        volume,
        cut: false,
        start
    })
    ...
    if (e.which == 191) { // slash key (toggle menus)
        ...
        if ([13, 32].includes(e.which)) { // enter, space
            ...
            if (ctrlHeld) {
                ...
                $('#sequence div:hover').first().trigger({
                    type: "wheel",
                    arrowDelta: e.which == 40 ? 69 : -69
                })
                e.preventDefault()
            }
...
$(document).on('change click', '.settingBox', function() {
    ...
    if (settingName == "oldSaving") {
        ...
        if (settings.proMode) {
            ...
            if (!ctrlHeld) $('#everything').animate({
                scrollTop: pageHeight + 1000
            })
        } else {
            $('#proHotbar').hide()
            $('#extraPadding').hide()
